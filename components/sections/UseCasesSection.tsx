...
const useCases = [
  {
    icon: ShieldAlert,
    title: "Infected critical bone defects",
    description: "Osteomyelitis cases requiring staged debridement, antibiotic spacer support, and delayed reconstruction.",
  },
  {
    icon: Zap,
    title: "Blast and high-energy trauma",
    description: "Complex limb injuries with segmental bone loss requiring temporary internal stabilisation.",
  },
  {
    icon: Scissors,
    title: "Oncologic segmental resection",
    description: "Post-resection defects needing stable internal fixation during staged limb salvage.",
  },
];
    ...
    <section id="use-cases" className="relative z-0 py-20 md:py-28 bg-white overflow-hidden">
      ...
      <div className="section-container relative z-10">
        <AnimatedSection>
          <SectionHeader
            label="Use Cases"
            title={<>Primary clinical{" "}<span className="text-teal-600">use cases</span></>}
          />
        </AnimatedSection>
            ...
            <StaggerItem key={i}>
              ...
              >
                <div className="w-11 h-11 rounded-xl bg-teal-50 border border-teal-100 flex items-center justify-center mb-5">
                  <uc.icon className="w-5 h-5 text-teal-600" />
                </div>
